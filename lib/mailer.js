...
module.exports = function(config) {
  ...
  if(config.getItem('testMode.noEmail')) {
    ...
    transporter = nodemailer.createTransport('SMTP', config.getItem('mailer.options'));
  }
  ...
  this.sendEmail = function(templateName, email, locals) {
    ...
    var body = ejs.render(template, locals, {filename: templateFile});
    // form the email
    ...
    // send the message
    var sendEmail = BPromise.promisify(transporter.sendMail, transporter);
